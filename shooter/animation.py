...
class AnimateSprite(pygame.sprite.Sprite):
    ...
    def __init__(self, sprite_name, size=((200, 200))):
        super().__init__()
        self.size = size
        self.image = pygame.image.load(f"html/python/assets/{sprite_name}.png")
        self.image = pygame.transform.scale(self.image, size)
        self.current_image =  0 # commencer l'ainm a l'image 0
        self.images = animations.get(sprite_name)
        self.animation = False
        
    # definir une methode pour demarrer l'animation
    def start_animaion(self):
        self.animation = True
        
    # definir une methode pour animer le sprite
    def animate(self, loop=False):
        
        # verrifier si l'animation est active
        if self.animation:
        
            # passer a l'image suivante
            self.current_image += 1
            
            # verifier si no a attaeit la fin de l'animation
            if self.current_image >= len(self.images):
                # remmetre l'animation au depart
                self.current_image = 0
                
                # verifier si l'animationn'est pas en mode boucle
...
def load_animation_images(sprite_name):
    ...
    images = []
    # recuperer le chemin du dossier de ce sprite
    path = f"html/python/assets/{sprite_name}/{sprite_name}"
...
# definir un dictionnaire qui va chargees les images de chaque sprite
# mummy -> [...player1, ...player2, ...]
animations = {
